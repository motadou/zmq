...
{
public:
    explicit decoder_base_t (const size_t buf_size_) : _next (NULL), _read_pos (NULL), _to_read (0), _allocator(buf_size_)
    {
    ...
    //  get_buffer function. 
    //  size_ argument specifies number of bytes actually filled into the buffer. 
